...
describe("Idempotency Manager", () => {
  ...
  test("commitError stores result with a shorter TTL than full result TTL (NF-05)", async () => {
    const key = globalIdempotencyManager.generateKey("tenant-err", "err_tool", { test: true });
