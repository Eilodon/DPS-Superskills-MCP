...
describe("MCP server card", () => {
  test("publishes tool annotations and execution metadata", () => {
    ...
    // MISS-4/I-4.3: pluginTrustBoundary, patternDebt, safeMode and friends are
    // deliberately NOT exposed in the server card — they provide attacker
    // reconnaissance. Only non-sensitive capability signals remain.
    expect(card._meta.security.pluginTrustBoundary).toBeUndefined();
    expect(card._meta.security.patternDebt).toBeUndefined();
    expect(card._meta.security.toolCount).toBe(tools.length);
  });
