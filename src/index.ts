...
async function main() {
  ...
  console.error(`[DPS-Superskills] Server Started v1.0.0`);
  console.error(`[DPS-Superskills] Tenant: ${ENV.MCP_TENANT_ID} | Project: ${ENV.MCP_PROJECT_ID}`);
  console.error(`[DPS-Superskills] Config: Transport=${ENV.TRANSPORT_DRIVER}, Storage=${ENV.STORAGE_DRIVER}, Telemetry=${ENV.TELEMETRY_DRIVER}`);
  console.error(`[DPS-Superskills] Security: Encrypted=${!!ENV.MCP_ENCRYPTION_KEY}, SafeMode=${ENV.MCP_SAFE_MODE}`);
  console.error(`[DPS-Superskills] Current Phase: ${state.phase}`);
